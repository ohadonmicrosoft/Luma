...
function getRecentlyModifiedFiles() {
  ...
  try {
    const gitOutput = execSync("git diff --name-only HEAD~10 HEAD", {
      encoding: "utf8",
    });
    return gitOutput
      .split("\n")
    ...
    console.warn(
      "Could not get recently modified files from git, falling back to all files"
    );
    return [];
  }
}
...
console.log(
  `🔧 Processing ${filesToLint.length} files in ${batches.length} batches`
);
...
batches.forEach((batch, index) => {
  ...
  const lintCommand = `npx eslint ${batch.join(" ")} ${
    config.fix ? "--fix" : ""
  }`;
  ...
  try {
    ...
    if (config.fix) {
      ...
      const fixedMatch = output.match(
        /(\d+) problems? \((\d+) errors?, (\d+) warnings?\)[\s\S]*?(\d+) errors? fixed/
      );
    ...
    if (!process.argv.includes("--continue-on-error")) {
      console.error(
        "Stopping due to errors. Use --continue-on-error to ignore errors."
      );
      process.exit(1);
    }
  }
