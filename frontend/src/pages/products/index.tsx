...
import {
  ProductResultsGrid,
  SortOption,
} from "@/components/products/ProductResultsGrid";
...
const mockProducts = Array.from({ length: 24 }).map((_, index) => ({
  ...
  currency: "USD",
  discountedPrice:
    Math.random() > 0.7 ? 500 + Math.floor(Math.random() * 1000) : undefined,
  ...
  specifications: Array.from({ length: 3 }).map((_, specIndex) => ({
    ...
    name: {
      en:
        ["Resolution", "Memory", "Processor"][specIndex] || `Spec ${specIndex}`,
...
export default function ProductsPage({
  ...
  const [filterCategories, setFilterCategories] =
    useState<FilterCategory[]>(mockFilterCategories);
  ...
  const countActiveFilters = () => {
    ...
    filterCategories.forEach((category) => {
      ...
      category.rangeFilters?.forEach((filter) => {
        if (
          filter.rangeValues[0] > filter.min ||
          filter.rangeValues[1] < filter.max
        ) {
          count += 1;
        }
      });
  ...
  const handleFilterChange = (
    ...
    setFilterCategories((prevCategories) => {
      ...
      if (typeof value === "object" && value !== null && "optionId" in value) {
        ...
        const selectionFilterIndex = newCategories[
          categoryIndex
        ].selectionFilters?.findIndex((f) => f.id === filterId);

        if (selectionFilterIndex === undefined || selectionFilterIndex === -1)
          return prevCategories;

        const filter =
          newCategories[categoryIndex].selectionFilters![selectionFilterIndex];
      ...
      // Handle range filters
        ...
        const rangeFilterIndex = newCategories[
          categoryIndex
        ].rangeFilters?.findIndex((f) => f.id === filterId);

        if (rangeFilterIndex === undefined || rangeFilterIndex === -1)
          return prevCategories;
        ...
        if (newCategories[categoryIndex].rangeFilters) {
          ...
          newCategories[categoryIndex].rangeFilters![
            rangeFilterIndex
          ].rangeValues = typedValue;
        }
      }
