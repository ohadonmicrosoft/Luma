...
export const ProductComparison: React.FC<ProductComparisonProps> = ({
  ...
  function hasDifferentValues(products: Product[], specId: string): boolean {
    const values = products
      .map(product => product.specifications.find(s => s.id === specId)?.value)
      ...
      <Card className={cn("max-w-3xl mx-auto", className)}>
        <CardContent className="p-8 text-center">
          <p className="text-neutral-500">
            {isRTL ? 'אין מוצרים להשוואה. אנא בחר מוצרים להשוואה.' : 'No products to compare. Please select products to compare.'}
          </p>
        </CardContent>
      </Card>
    );
        ...
        <div className="min-w-max">
          ...
          <div className="grid grid-cols-[200px_repeat(auto-fill,minmax(180px,1fr))] border-b border-neutral-200">
            ...
            {products.map(product => (
              <div key={product.id} className="p-4 text-center border-l border-neutral-200">
                ...
                <h3 className="font-medium text-neutral-800 mb-1">
                  <Link href={`/products/${product.slug}`} className="hover:text-primary-600 hover:underline">
                ...
                <div className="flex justify-center items-center gap-2 mb-1">
                  {product.discountedPrice !== undefined && (
                    <span className="text-red-600 font-medium">
                      {formatPrice(product.discountedPrice, product.currency || Currency.USD)}
                    </span>
                  )}
                  <span className={cn(
                    product.discountedPrice !== undefined ? "text-neutral-500 line-through text-xs" : "text-neutral-900"
                  )}>
                    {formatPrice(product.price, product.currency || Currency.USD)}
                  </span>
                </div>
          ...
          {groupedSpecifications.map(category => (
            <div key={category.id}>
              ...
              <div className="grid grid-cols-[200px_repeat(auto-fill,minmax(180px,1fr))] bg-neutral-100">
                ...
                {products.map(product => (
                  <div key={product.id} className="p-3 border-l border-neutral-200"></div>
                ))}
              </div>
              ...
              {category.specs.map(spec => (
                ...
                >
                  ...
                  {products.map(product => {
                    ...
                    return (
                      <div key={product.id} className="p-3 border-l border-neutral-200 text-center">
                        {productSpec ? (
                          <span className={cn(
                            "font-medium",
                            hasDifferentValues(products, spec.id) && "text-primary-700"
                          )}>
                            {productSpec.unit
