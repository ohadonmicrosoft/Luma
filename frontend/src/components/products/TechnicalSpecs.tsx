...
export const SpecificationItem: React.FC<{
  ...
  const formattedValue = spec.unit && typeof spec.value === 'number'
  ...
  return (
    <div 
      className={cn(
        ...
        spec.isHighlight && "font-medium text-primary-700 bg-primary-50 px-2 -mx-2 rounded",
...
export const TechnicalSpecs: React.FC<TechnicalSpecsProps> = ({
  ...
  const hasCategories = specifications.length > 0 && 'specs' in specifications[0];
      ...
      <CardHeader className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className={cn(
          ...
          {onToggleHighlights && (
            <div className="flex items-center gap-2">
              <Checkbox 
                ...
                onCheckedChange={(checked: boolean) => onToggleHighlights(checked)}
              />
              <label 
                htmlFor="highlights" 
          ...
          {canCompare && onCompareToggle && (
            ...
            >
              {isComparing 
                ? (isRTL ? 'מסומן להשוואה' : 'Selected for comparison') 
                : (isRTL ? 'הוסף להשוואה' : 'Add to comparison')}
            </Button>
          )}
        </div>
      </CardHeader>
