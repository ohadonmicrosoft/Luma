    ...
    return (
      <div className="w-full py-12 flex flex-col items-center justify-center text-center">
        ...
        <p className="text-neutral-500">
          {isRTL
            ? "נסה לחפש משהו אחר או לשנות את הסינון שלך"
    ...
    return (
      <div className="relative">
        <Button
          ...
          size="sm"
          onClick={() =>
            setHoveredSortId(hoveredSortId ? null : "sort-dropdown")
          }
          className="flex items-center gap-1"
        >
          <span>{isRTL ? "מיון" : "Sort"}</span>
          <svg
            className={`w-4 h-4 transition-transform ${
              hoveredSortId === "sort-dropdown" ? "rotate-180" : ""
            }`}
            fill="none"
            viewBox="0 0 24 24"
            stroke="currentColor"
  ...
  return (
    <div className={cn("flex flex-col", className)}>
      ...
      <div className="flex flex-wrap justify-between items-center mb-4 gap-2">
        ...
        <div className="flex items-center gap-2">{renderSortOptions()}</div>
      </div>
      ...
      {products.length === 0 && !loading ? (
        ...
        <div className={cn("grid gap-4 md:gap-6", getGridColumnClass())}>
          {loading
            ? renderLoadingState()
