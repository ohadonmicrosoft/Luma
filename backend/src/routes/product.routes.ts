...
router.get(
  "/featured",
  productController.getFeaturedProducts as RequestHandler
);
router.get(
  "/category/:categoryId",
...
router.get(
  "/technical-specs",
  productController.filterByTechnicalSpecs as RequestHandler
);
router.get("/:id", productController.getProductById as RequestHandler);
router.get(
  "/:id/related",
  productController.getRelatedProducts as RequestHandler
);
router.get(
  "/:id/compatible",
  productController.getCompatibleProducts as RequestHandler
);
...
router.patch(
  ...
  checkRole([UserRole.Admin]) as RequestHandler,
  validateRequest(
    productValidationSchemas.updateTechnicalSpecs
  ) as RequestHandler,
