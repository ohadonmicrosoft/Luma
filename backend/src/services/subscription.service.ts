...
export class SubscriptionService {
  ...
  constructor() {
    ...
    this.subscriptionItemRepository = AppDataSource.getRepository(SubscriptionItem);
  ...
  async getSubscriptionsByUserId(userId: string): Promise<Subscription[]> {
    try {
      ...
      throw new AppError('Failed to get subscriptions', StatusCode.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async getSubscriptionById(id: string): Promise<Subscription> {
    try {
      ...
      throw new AppError('Failed to get subscription', StatusCode.INTERNAL_SERVER_ERROR);
    }
  }
    ...
    try {
      ...
      return await AppDataSource.transaction(async transactionalEntityManager => {
        ...
        for (const itemInput of dto.items) {
          ...
          if (!product) {
            throw new AppError(`Product with ID ${itemInput.product_id} not found`, StatusCode.NOT_FOUND);
          }

          if (product.stock < itemInput.quantity) {
            throw new AppError(`Not enough stock for product ${product.name}`, StatusCode.BAD_REQUEST);
          }
          ...
          await transactionalEntityManager.save(SubscriptionItem, subscriptionItem);
      ...
      throw new AppError('Failed to create subscription', StatusCode.INTERNAL_SERVER_ERROR);
    }
  }
   ...
   */
  async updateFrequency(id: string, frequency: SubscriptionFrequency): Promise<Subscription> {
    try {
      const subscription = await this.getSubscriptionById(id);

      // Check if subscription can be updated
      if (subscription.status !== SubscriptionStatus.ACTIVE) {
        throw new AppError('Only active subscriptions can be updated', StatusCode.BAD_REQUEST);
      }
      ...
      throw new AppError('Failed to update subscription frequency', StatusCode.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async updateAutoRenew(id: string, autoRenew: boolean): Promise<Subscription> {
    try {
      ...
      throw new AppError('Failed to update subscription auto-renew setting', StatusCode.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async pauseSubscription(id: string): Promise<Subscription> {
    try {
      ...
      if (subscription.status !== SubscriptionStatus.ACTIVE) {
        throw new AppError('Only active subscriptions can be paused', StatusCode.BAD_REQUEST);
      }
      ...
      throw new AppError('Failed to pause subscription', StatusCode.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async resumeSubscription(id: string): Promise<Subscription> {
    try {
      ...
      if (subscription.status !== SubscriptionStatus.PAUSED) {
        throw new AppError('Only paused subscriptions can be resumed', StatusCode.BAD_REQUEST);
      }
      ...
      throw new AppError('Failed to resume subscription', StatusCode.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async cancelSubscription(id: string): Promise<Subscription> {
    try {
      ...
      if (subscription.status === SubscriptionStatus.CANCELLED) {
        throw new AppError('Subscription is already cancelled', StatusCode.BAD_REQUEST);
      }
      ...
      throw new AppError('Failed to cancel subscription', StatusCode.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async addProductToSubscription(
    ...
    try {
      // Start a transaction
      return await AppDataSource.transaction(async transactionalEntityManager => {
        const subscription = await transactionalEntityManager.findOne(Subscription, {
          where: { id },
          relations: ['items', 'items.product']
        });
        ...
        const existingItem = subscription.items.find(item => item.product_id === productId);

        if (existingItem) {
          // Update quantity
          existingItem.quantity += quantity;
          existingItem.selected_options = selectedOptions || existingItem.selected_options;
          await transactionalEntityManager.save(SubscriptionItem, existingItem);
        } else {
          // Create new subscription item
          const newItem = transactionalEntityManager.create(SubscriptionItem, {
            subscription_id: id,
      ...
      throw new AppError('Failed to add product to subscription', StatusCode.INTERNAL_SERVER_ERROR);
    }
  }
   ...
   */
  async removeProductFromSubscription(id: string, itemId: string): Promise<Subscription> {
    try {
      // Start a transaction
      return await AppDataSource.transaction(async transactionalEntityManager => {
        const subscription = await transactionalEntityManager.findOne(Subscription, {
        ...
        // Find the item
        const itemIndex = subscription.items.findIndex(item => item.id === itemId);

        if (itemIndex === -1) {
          throw new AppError('Item not found in subscription', StatusCode.NOT_FOUND);
        }
        ...
        if (subscription.items.length === 0) {
          throw new AppError('Subscription must have at least one item', StatusCode.BAD_REQUEST);
        }
      ...
      throw new AppError('Failed to remove product from subscription', StatusCode.INTERNAL_SERVER_ERROR);
    }
  }
   ...
   */
  async updateItemQuantity(id: string, itemId: string, quantity: number): Promise<Subscription> {
    try {
      if (quantity <= 0) {
      ...
      logger.error(`Error updating item quantity in subscription ${id}:`, error);
      throw new AppError('Failed to update item quantity', StatusCode.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async updateShippingAddress(
    ...
    try {
      ...
      logger.error(`Error updating shipping address for subscription ${id}:`, error);
      throw new AppError('Failed to update shipping address', StatusCode.INTERNAL_SERVER_ERROR);
    }
  }
  ...
  async processRenewals(): Promise<void> {
    try {
      ...
      throw new AppError('Failed to process subscription renewals', StatusCode.INTERNAL_SERVER_ERROR);
    }
  }
} 
